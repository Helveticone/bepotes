...
- **Cache-busting** : `theme.js` injecté dans le `<head>` (avant rendu, anti-flash) ; versions actuelles `app.supabase.js?v=77`, `style.css?v=79`, `nav.js?v=6`, `bell.js?v=23`, `theme.js?v=1`, `pwa.js?v=21`, `push.js?v=1`, `install.js?v=1`, `stories.js?v=3`. `config.js` (no-cache, pas de `?v`) contient `VAPID_PUBLIC`, `MEDIA_CDN`, `TURNSTILE_SITEKEY`, `TENOR_KEY` (tous activables au lancement).
